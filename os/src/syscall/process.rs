...
pub fn sys_writev(
    ...
    for i in 0..iovcnt {
        let buf = translated_ref(current_user_token(), iov.wrapping_add(i));
        count += sys_write(fdd, buf.iov_base, buf.iov_len) as isize;
    }
    count 
}

pub fn sys_exit_group() -> isize {
    0
}